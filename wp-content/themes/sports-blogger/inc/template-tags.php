 ...
 */
if ( ! function_exists( 'sports_blogger_posted_on' ) ) :
	/**
	 * Prints HTML with meta information for the current post-date/time.
	 */
	function sports_blogger_posted_on() {
...
if ( ! function_exists( 'sports_blogger_time' ) ) {
	function sports_blogger_time() {
		$time_string = '<time class="entry-date published updated" datetime="%1$s">%2$s</time>';
		if ( get_the_time( 'U' ) !== get_the_modified_time( 'U' ) ) {
			$time_string = '<time class="entry-date published" datetime="%1$s">%2$s</time><time class="updated" datetime="%3$s">%4$s</time>';
		}
...
if ( ! function_exists( 'sports_blogger_posted_by' ) ) :
	function sports_blogger_posted_by( $author_image = true ) {
		...
		if ( false === $author_image ) {
			$get_author_image = __( 'Posted by', 'sports-blogger' );
		}
		$postedBy = sprintf(
			$posted_by_format,
...
if ( ! function_exists( 'sports_blogger_comment_popuplink' ) ) {
	function sports_blogger_comment_popuplink() {
		if ( ! is_single() && ! post_password_required() && ( comments_open() || get_comments_number() ) ) {
			...
			comments_popup_link(
				__( 'Post a Comment', 'sports-blogger' ),
				__( '1 Comment', 'sports-blogger' ),
				__( '% Comments', 'sports-blogger' ),
				$css_class,
				__( 'Comments are Closed', 'sports-blogger' )
			);
			echo '</span>';
		}
	}
}
function sports_blogger_categories() {
...
if ( ! function_exists( 'sports_blogger_post_tag' ) ) {
	function sports_blogger_post_tag() {
...
if ( ! function_exists( 'sports_blogger_post_thumbnail' ) ) :
	/**
	 ...
	 */
	function sports_blogger_post_thumbnail() {
		$get_blog_layout = get_theme_mod( 'blog_layout', 'grid' );
		$thumbnail_size  = 'sports-blogger-grid-thumbnail';
		if ( is_single() || is_page() ) {
			$thumbnail_size = 'sports-blogger-thumbnail-large';
		} else {
...
function sports_blogger_navigation() {
